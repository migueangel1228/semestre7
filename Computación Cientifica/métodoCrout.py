...
def main():
    ...
    print_matriz("Matriz A (Original)", A)
    print_matriz("Matriz L (Triangular Inferior)", L)
    print_matriz("Matriz U (Triangular Superior)", U)
    
    # Verificación L * U
    n = len(A)
    LU = [[sum(L[i][k] * U[k][j] for k in range(n)) for j in range(n)] for i in range(n)]
    print_matriz("Verificación (L * U)", LU)
